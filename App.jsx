...
const App = () => {
	...
	const loginReducer = (prevState, action) => {
		switch (action.type) {
			case 'RETRIEVE_TOKEN':
				return { ...prevState, id: action.id, nombre_completo: action.nombre_completo, token: action.token, isLoading: false };
			case 'LOGIN':
				return { ...prevState, id: action.id, nombre_completo: action.nombre_completo, token: action.token, isLoading: false };
			case 'LOGOUT':
				return { ...prevState, id: null, nombre_completo: null, token: null, isLoading: false };
		}
	};

	const [loginState, dispatch] = useReducer(loginReducer, { id: null, nombre_completo: null, token: null, isLoading: true });
	...
	const authContext = useMemo(() => ({
		signIn: async (foundUser) => {
			...
			const dataToken = {
				...
				nombre_completo: String(foundUser.nombre_completo),
	...
	useEffect(() => {
		setTimeout(async () => {
			...
			if (dataToken) {
				...
				dispatch({ type: 'RETRIEVE_TOKEN', id: null, nombre_completo: null, token: null, isLoading: false });
			}
		}, 2000);
	}, []);
